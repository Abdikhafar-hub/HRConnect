...
export default function EmployerJobsPage() {
  ...
  const [dialog, setDialog] = useState<{ type: null | "details" | "contact" | "progress"; job: Job | null }>({ type: null, job: null })

  // Get employerId from localStorage (or from auth context if you have it)
  const employerId = typeof window !== 'undefined' ? localStorage.getItem('employerId') : null
  ...
  const postedJobs = jobs.filter(job => job.status === "open")
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Jobs Tabs */}
        <Tabs defaultValue="active" className="space-y-8">
          ...
          {activeJobs.map((job) => (
              <Card key={job._id || job.title} className="shadow-xl rounded-2xl border-l-4 border-l-green-500 bg-white/80 backdrop-blur-md hover:shadow-2xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <Avatar className="w-12 h-12">
                      ...
                      <AvatarFallback>
                        {job.worker ? job.worker.split(" ").map((n) => n[0]).join("") : ""}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Dialog open={dialog.type === "progress" && dialog.job?._id === job._id} onOpenChange={open => !open && setDialog({ type: null, job: null })}>
                    ...
                    <Dialog open={dialog.type === "contact" && dialog.job?._id === job._id} onOpenChange={open => !open && setDialog({ type: null, job: null })}>
                      <DialogTrigger asChild>
                        ...
                        <div className="my-4 space-y-2">
                          <div className="flex items-center gap-2">
                            <Avatar className="w-10 h-10">
                              <AvatarImage src={job.workerPhoto || "/placeholder.svg"} />
                              <AvatarFallback>{job.worker ? job.worker.split(" ").map((n) => n[0]).join("") : ""}</AvatarFallback>
                            </Avatar>
                            <div>
                    ...
                    <Dialog open={dialog.type === "details" && dialog.job?._id === job._id} onOpenChange={open => !open && setDialog({ type: null, job: null })}>
                      <DialogTrigger asChild>
                        ...
                        <div className="my-4 space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-500">Rating: {job.rating !== undefined ? job.rating : '-'}</div>
                            </div>
                          </div>
                        </div>
                        <DialogFooter>
          ...
          {postedJobs.map((job) => (
              <Card key={job._id || job.title} className="shadow-xl rounded-2xl border-l-4 border-l-blue-500 bg-white/80 backdrop-blur-md hover:shadow-2xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{job.title}</h3>
          ...
          {completedJobs.map((job) => (
              <Card key={job._id || job.title} className="shadow-xl rounded-2xl border-l-4 border-l-purple-500 bg-white/80 backdrop-blur-md hover:shadow-2xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <Avatar className="w-12 h-12">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{job.worker}</p>
                      <div className="flex items-center space-x-1 mt-1">
                        <span className="text-sm text-gray-500">Worker Rating: {job.workerRating !== undefined ? job.workerRating : '-'}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="flex items-center text-yellow-600">
                    <span className="text-sm">You rated: {job.rating !== undefined ? job.rating : '-'}/5</span>
                  </div>
                </div>
                  <div className="flex flex-wrap gap-2">
